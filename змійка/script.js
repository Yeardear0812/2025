...
// Керування для мобільних пристроїв
upBtn.addEventListener('click', () => { if (!isPaused && dy !== 1) { dx = 0; dy = -1; } });
downBtn.addEventListener('click', () => { if (!isPaused && dy !== -1) { dx = 0; dy = 1; } });
leftBtn.addEventListener('click', () => { if (!isPaused && dx !== 1) { dx = -1; dy = 0; } });
rightBtn.addEventListener('click', () => { if (!isPaused && dx !== -1) { dx = 1; dy = 0; } });
